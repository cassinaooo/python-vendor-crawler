...
class GenericSpider(CrawlSpider):
    ...
    with open(
        os.path.join(
            "/".join(dirname.split("/")), "data/", "in/", "words_dictionary.json"
        )
    ) as json_file:
        lang_dictionary = json.load(json_file)

    @classmethod
    def create(cls, link, f_name):
        """
        ...
        class_name = (
            domain if not domain.startswith("www.") else domain[4:]
        ).title().replace(".", "") + cls.__name__
        return type(
            class_name,
            (cls,),
            {
                ...
                "start_urls": [link],
                "name": domain,
                "custom_settings": {"FEED_URI": f_name},
            },
        )
    ...
    def parse_item(self, response):
        ...
        web_text = "".join(
            response.xpath(
                "//*[not(self::script or self::style or self::footer)]/text()"
            ).extract()
        )
        ...
        # filter for english
        english_web_text = " ".join(
            [w for w in web_text.split() if w in self.lang_dictionary]
        )

        item["company_text"] = english_web_text
        item["company_name"] = self.custom_settings["FEED_URI"]
